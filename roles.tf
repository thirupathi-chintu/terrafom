...
data "aws_iam_policy_document" "jenkins_task_execution_role" {
  version = "2012-10-17"
  statement {
    sid = ""
    effect = "Allow"
...
resource "aws_iam_role" "jenkins_task_execution_role" {
  name               = var.jenkins_task_execution_role_name
  assume_role_policy = data.aws_iam_policy_document.jenkins_task_execution_role.json
}

# ECS task execution role policy attachment
resource "aws_iam_role_policy_attachment" "jenkins_task_execution_role" {
